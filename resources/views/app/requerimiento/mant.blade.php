...
{!! Form::model($movimiento, $formData) !!}	
    ...
    <div class="row">
        <div class="col-lg-6 col-md-6 col-sm-6">
            ...
            <div class="form-group">
        		{!! Form::label('persona', 'Personal:', array('class' => 'col-lg-2 col-md-2 col-sm-2 control-label')) !!}
        		<div class="col-lg-9 col-md-9 col-sm-9">
                {!! Form::hidden('persona_id', $persona_id, array('id' => 'persona_id')) !!}
                {!! Form::hidden('ruc', '', array('id' => 'ruc')) !!}
        		{!! Form::text('persona', $persona, array('class' => 'form-control input-xs', 'id' => 'persona', 'placeholder' => 'Ingrese Personal')) !!}
        		</div>
         ...
         <div class="col-lg-6 col-md-6 col-sm-6" >
            <div class="form-group" style="">
                {!! Form::label('codigo', 'Cod. Barra:', array('class' => 'col-lg-2 col-md-2 col-sm-2 control-label','style'=>'display:none')) !!}
...
$(document).ready(function() {
    ...
    var personas2 = new Bloodhound({
		...
		remote: {
			url: 'requerimiento/personautocompletar/%QUERY',
...
function agregarDetalle(id){
    $.ajax({
        type: "POST",
        url: "requerimiento/agregardetalle",
        data: "id="+id+"&_token="+$(IDFORMMANTENIMIENTO + '{!! $entidad !!} :input[name="_token"]').val(),
        success: function(a) {
            datos=JSON.parse(a);
            for(d=0;d < datos.length; d++){
                console.log(datos[d]);
                //datos[d].idproducto="01"+Math.round(Math.random()*100);
                $("#tbDetalle").append("<tr id='tr"+datos[d].id+"'><td><input type='hidden' id='txtIdDetalle"+datos[d].id+"' name='txtIdDetalle"+datos[d].id+"' value='"+datos[d].id+"' /><input type='hidden' id='txtIdProducto"+datos[d].id+"' name='txtIdProducto"+datos[d].id+"' value='"+datos[d].idproducto+"' /><input type='text' data='numero' style='width: 40px;' class='form-control input-xs' id='txtCantidad"+datos[d].id+"' name='txtCantidad"+datos[d].id+"' value='"+datos[d].cantidad+"' size='3' /></td>"+
                        "<td align='left'>"+datos[d].producto+"</td>"+
