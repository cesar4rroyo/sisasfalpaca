					...
					<div class="row">
						<div class="col-xs-12">
							...
							<div class="form-group">
								...
								{!! Form::text('banco', '', array('class' => 'form-control input-xs', 'id' => 'banco')) !!}
							</div>
							<div class="form-group">
								{!! Form::label('comentario', 'Comentario:') !!}
								{!! Form::text('comentario', '', array('class' => 'form-control input-xs', 'id' => 'comentario')) !!}
							</div>
							<div class="form-group">
...
<script>
	$(document).ready(function () {
		...
		$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="comentario"]').keyup(function (e) {
			var key = window.event ? e.keyCode : e.which;
			if (key == '13') {
				buscar('{{ $entidad }}');
			}
	...
	function reporte(entidad){
	    window.open("otrascompra/pdf?fechainicio="+$('#fechainicio').val()+"&fechafin="+$("#fechafin").val()+"&persona="+$("#persona").val()+"&comentario="+$("#comentario").val()+"&banco="+$("#banco").val()+"&situacion="+$("#situacion").val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
	}
