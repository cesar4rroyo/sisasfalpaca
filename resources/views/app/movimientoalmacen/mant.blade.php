    ...
    <div class="row">
        <div class="col-lg-7 col-md-7 col-sm-7">
            ...
            <div class="form-group" style="">
        		{!! Form::label('persona', 'Resp.:', array('class' => 'col-lg-1 col-md-1 col-sm-1 control-label')) !!}
            ...
            <div class="form-group">
                {!! Form::label('comentario', 'Motivo:', array('class' => 'col-lg-1 col-md-1 col-sm-1 control-label')) !!}
                <div class="col-lg-3 col-md-3 col-sm-3">
                    {!! Form::textarea('comentario', '', array('class' => 'form-control input-xs', 'id' => 'comentario', 'rows' => '3')) !!}
                </div>
                {!! Form::label('almacen_id', 'Almacen:', array('class' => 'col-lg-1 col-md-1 col-sm-1 control-label')) !!}
                <div class="col-lg-3 col-md-3 col-sm-3">
                    {!! Form::select('almacen_id', $cboAlmacen, null, array('class' => 'form-control input-xs', 'id' => 'almacen_id')) !!}
                </div>
                {!! Form::label('destino', 'Destino:', array('class' => 'col-lg-1 col-md-1 col-sm-1 control-label salida', 'style' => 'display:none')) !!}
                <div class="col-lg-3 col-md-3 col-sm-3 salida" style='display:none'>
                    {!! Form::select('destino', $cboDestino, null, array('class' => 'form-control input-xs', 'id' => 'destino')) !!}
                </div>
            </div>
        	<div class="form-group">
        	    {!! Form::label('maquinaria_id', 'Maquinaria:', array('class' => 'col-lg-2 col-md-2 col-sm-2 control-label')) !!}
        		<div class="col-lg-4 col-md-4 col-sm-4">
        			{!! Form::select('maquinaria_id', $cboMaquinaria, null, array('class' => 'form-control input-xs', 'id' => 'maquinaria_id')) !!}
        		</div>
         ...
         <div class="col-lg-5 col-md-5 col-sm-5">
            <div class="form-group">
                {!! Form::label('codigo', 'Cod. Barra:', array('class' => 'col-lg-2 col-md-2 col-sm-2 control-label', 'hidden' => '')) !!}
...
$(document).ready(function() {
    ...
    var personas2 = new Bloodhound({
		...
		remote: {
			url: 'movimientoalmacen/personautocompletar/%QUERY',
...
function guardarPago (entidad, idboton) {
    ...
    if($("#tipodocumento").val()=="9"){
        if($("#almacen_id").val()==$("#destino").val()){
            band = false;
            msg += " *Almacen y el destino no pueden ser iguales \n";
        }
    }
    if(band && contador==0){
        contador=1;
...
function buscarProducto(valor){
    $.ajax({
        ...
        data: "descripcion="+$(IDFORMMANTENIMIENTO + '{!! $entidad !!} :input[id="descripcion"]').val()+"&almacen_id="+$(IDFORMMANTENIMIENTO + '{!! $entidad !!} :input[id="almacen_id"]').val()+"&_token="+$(IDFORMMANTENIMIENTO + '{!! $entidad !!} :input[name="_token"]').val(),
        success: function(a) {
            datos=JSON.parse(a);
            $("#divBusqueda").html("<table class='table table-bordered table-condensed table-hover' border='1' id='tablaProducto'><thead><tr><th class='text-center'>PRODUCTO</th><th class='text-center'>STOCK</th></tr></thead></table>");
            var pag=parseInt($("#pag").val());
            var d=0;
            for(c=0; c < datos.length; c++){
                var a="<tr id='"+datos[c].idproducto+"' onclick=\"seleccionarProducto('"+datos[c].idproducto+"','"+datos[c].codigobarra+"','"+datos[c].producto+"','"+datos[c].preciocompra+"','"+datos[c].precioventa+"','"+datos[c].stock+"')\"><td>"+datos[c].producto+"</td><td align='right'>"+datos[c].stock+"</td></tr>";
...
function generarNumero(){
    $.ajax({
        ...
        success: function(a) {
            ...
            if($(IDFORMMANTENIMIENTO + '{!! $entidad !!} :input[name="tipodocumento"]').val()=="9"){
                $(".salida").css('display','');
            }else{
