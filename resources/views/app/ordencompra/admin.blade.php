...
function pdf(id){
    window.open("ordencompra/pdf?id="+id+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
}

function reporte(entidad){
	window.open("ordencompra/pdfReporte?fechainicio="+$('#fechainicio').val()+"&fechafin="+$("#fechafin").val()+"&persona="+$("#persona").val()+"&numero="+$("#numero").val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
}
</script>