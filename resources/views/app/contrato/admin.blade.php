				...
				<div class="box-header">
					<div class="row">
						<div class="col-xs-12">
							...
							{!! Form::button('<i class="glyphicon glyphicon-file"></i> Excel Avance', array('class' => 'btn btn-warning btn-xs', 'id' => 'btnPdf', 'onclick' => 'excel(\''.$entidad.'\')')) !!}
							{!! Form::close() !!}
						</div>
					</div>
				</div>
...
function word(id){
    window.open("contrato/word?id="+id+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
}
function excel(entidad){
    window.open("contrato/excel?fechainicio="+$('#fechainicio').val()+"&fechafin="+$("#fechafin").val()+"&cliente="+$("#cliente").val()+"&numero="+$("#numero").val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
}
</script>