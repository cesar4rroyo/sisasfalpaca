					...
					<div class="row">
						<div class="col-xs-12">
							...
							<div class="form-group">
								...
								{!! Form::text('obra', '', array('class' => 'form-control input-xs', 'id' => 'obra')) !!}
							</div>
							<div class="form-group">
								{!! Form::label('maquinaria', 'Maquinaria:') !!}
								{!! Form::text('maquinaria', '', array('class' => 'form-control input-xs', 'id' => 'maquinaria')) !!}
							</div>
							<div class="form-group">
							...
							{!! Form::button('<i class="glyphicon glyphicon-file"></i> Excel', array('class' => 'btn btn-success btn-xs', 'id' => 'btnExcel', 'style' => 'display:none;', 'onclick' => 'excel(\''.$entidad.'\')')) !!}
...
<script>
	$(document).ready(function () {
		...
		$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="maquinaria"]').keyup(function (e) {
			var key = window.event ? e.keyCode : e.which;
			if (key == '13') {
				buscar('{{ $entidad }}');
			}
		});
		$(IDFORMBUSQUEDA + '{{ $entidad }} :input[id="obra"]').keyup(function (e) {
			var key = window.event ? e.keyCode : e.which;
			if (key == '13') {
				buscar('{{ $entidad }}');
			}
	...
	function excel(entidad){
        window.open("reporterequerimiento/excel?fechainicio="+$('#fechainicio').val()+"&fechafin="+$("#fechafin").val()+"&proveedor="+$("#proveedor").val()+"&tipodocumento="+$("#tipodocumento").val()+"&situacion="+$("#situacion").val()+"&modo="+$("#modo").val()+"&monto="+$("#monto").val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
	}
	function pdf(entidad){
        window.open("reporterequerimiento/pdf?fechainicio="+$('#fechainicio').val()+"&fechafin="+$("#fechafin").val()+"&proveedor="+$("#proveedor").val()+"&producto="+$("#producto").val()+"&obra="+$("#obra").val()+"&maquinaria="+$("#maquinaria").val()+"&placa="+$("#placa").val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
	}
