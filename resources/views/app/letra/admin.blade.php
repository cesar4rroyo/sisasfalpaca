				...
				<div class="box-header">
					<div class="row">
						<div class="col-xs-12">
							...
							{!! Form::button('<i class="glyphicon glyphicon-file"></i> Reporte', array('class' => 'btn btn-danger btn-xs', 'id' => 'btnReporte', 'onclick' => 'reporte2(\''.$entidad.'\')')) !!}
							{!! Form::close() !!}
						</div>
					</div>
				</div>
...
<script>
	...
	function reporte2(entidad){
	    window.open("letra/pdf?fechainicio="+$('#fechainicio').val()+"&fechafin="+$("#fechafin").val()+"&proveedor="+$("#proveedor").val()+"&banco="+$("#banco").val()+"&situacion="+$("#situacion").val()+"&numero="+$("#numero").val()+"&_token="+$(IDFORMBUSQUEDA + '{!! $entidad !!} :input[name="_token"]').val(),"_blank");
	}
</script>