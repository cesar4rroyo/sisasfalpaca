...
if(!is_null($cheque)){
    $fecha = $cheque->fecha;
    $fecha2 = $cheque->fechavencimiento;
    $tipodocumento = $cheque->tipodocumento_id;
    $detraccion = $cheque->incluye;
    $proveedor_id = $cheque->persona_id;
    $proveedor = trim($cheque->persona->razonsocial.' '.$cheque->persona->apellidopaterno.' '.$cheque->persona->apellidomaterno.' '.$cheque->persona->nombres);
    $montodetraccion = $cheque->detraccion;
    $porcentajedetraccion = round($cheque->detraccion*100/$cheque->total,2);
    $numero = is_null($cheque->numero)?'':$cheque->numero;
}else{
    $fecha = date("Y-m-d");
    $fecha2 = date("Y-m-d");
    $tipodocumento = null;
...
function guardarCheque (entidad, idboton) {
    ...
    for(c=0; c < carro.length; c++){
        if($("#txtFecha"+carro[c]).val()==""){
            band = false;
            msg += " *Fecha de Cheque "+$("#txtCantidad"+carro[c]).val()+" incorrecta \n";
        }
    }
    if(band && contador==0){
        contador=1;
...
function agregarItem(){
    ...
    $("#tbDetalle").append("<tr id='tr"+idproducto+"'><td><input type='hidden' id='txtIdProducto"+idproducto+"' name='txtIdProducto"+idproducto+"' value='"+idproducto+"' /><input type='text' data='numero' style='width: 40px;' class='form-control input-xs' id='txtCantidad"+idproducto+"' name='txtCantidad"+idproducto+"' value='"+(carro.length+1)+"' size='3' /></td>"+
            ...
            "<td align='left'><input type='date' id='txtFechaVencimiento"+idproducto+"' name='txtFechaVencimiento"+idproducto+"' class='form-control input-xs' /></td>"+
...
if(!is_null($cheque)){
    ...
    echo "mostrarDetraccion('".$cheque->incluye."');";
    echo $js;echo $js2;
